...
/* ⭐ GANZ GEMISCHT (22.09.2026) — ⛔ ÜBERHOLT ist die Sortierung nach Aktualität.
   Elias am 06.09.2026 wollte die Sätze eines Themas „nach aktualität", das
   Neueste zuerst. Am 22.09.2026, auf die Frage, ob jedes Mal dieselben Sätze
   zuerst kommen sollen: „immer unterschiedliche sätze" — und nachdem gemessen
   war, dass je Thema meist nur 1 bis 3 Sätze in der neuesten Gruppe stehen
   (vorn also fast immer dieselben), auf „Soll ich ganz mischen, auch wenn das
   Neueste dann nicht mehr zuerst kommt?": „ja".

   Der Zufallswert gilt je Satz für die ganze SITZUNG (bis die App neu startet):
   wer den Satzmodus verlässt und zurückkommt, findet seinen Platz wieder, statt
   dass alles unter ihm neu gewürfelt wird. Beim nächsten Start ist die
   Reihenfolge eine andere. Die THEMEN bleiben nach Aktualität sortiert
   (themenNachAktualitaet, sein Wunsch vom 26.08.) — gemischt werden nur die
   Sätze darin. */
const SATZ_ZUFALL = new Map();
function satzZufall(w){
  const k = (w && (w.id != null ? w.id : w.sentAr)) || '';
  if (!SATZ_ZUFALL.has(k)) SATZ_ZUFALL.set(k, Math.random());
  return SATZ_ZUFALL.get(k);
}
